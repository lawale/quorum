...
<aside class="w-64 bg-sidebar text-white flex flex-col min-h-screen">
  <div class="px-6 py-5">
    <h1 class="text-xl font-bold tracking-tight">Quorum</h1>
    <p class="text-[10px] font-medium uppercase tracking-[0.05em] text-white/40 mt-1">Admin Console</p>
  </div>
  ...
  <nav class="flex-1 px-3 py-4 space-y-1">
    {#each navItems as item}
      <a
        href={item.href}
        class="flex items-center px-3 py-2 text-sm rounded-md transition-colors {isActive(item.href) ? 'bg-sidebar-active text-white font-medium' : 'text-white/70 hover:bg-sidebar-hover hover:text-white'}"
      >
  ...
  <div class="px-3 py-4">
    ...
    <button
      ...
      class="w-full mt-2 flex items-center px-3 py-2 text-sm text-white/70 hover:bg-sidebar-hover hover:text-white rounded-md transition-colors"
    >
