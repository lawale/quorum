...
func setupPostgres(t *testing.T) *postgres.DB {
	...
	ctr, err := tcpostgres.Run(ctx, "postgres:16-alpine",
		...
		tcpostgres.WithInitScripts(
			filepath.Join(migrationsDir(), "001_create_policies.up.sql"),
			filepath.Join(migrationsDir(), "002_create_requests.up.sql"),
			filepath.Join(migrationsDir(), "003_create_approvals.up.sql"),
